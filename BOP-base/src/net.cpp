...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CService("158.69.26.138", 80); // wtfismyip.com

            if (nLookup == 1)
            {
                CService addrIP("wtfismyip.com/text", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "beonpush " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
	{"seed1.beonpush.com", "seed1.beonpush.com"},
	{"seed2.beonpush.com", "seed2.beonpush.com"},
	{"seed3.beonpush.com", "seed3.beonpush.com"},
	{"seed4.beonpush.com", "seed4.beonpush.com"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("beonpush-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. beonpush is probably already running."), addrBind.ToString().c_str());
        else
